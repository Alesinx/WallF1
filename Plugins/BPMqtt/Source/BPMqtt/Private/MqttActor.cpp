...
//static function
int UMqttActor::on_message_arrived(void* context, char* topicName, int topicLen, MQTTAsync_message* msg)
{
...
void UMqttActor::onfailure(void* context, MQTTAsync_failureData* response)
{
	fails.push_back(FailData{context, UTF8_TO_TCHAR(response->message) });
}

std::vector<UMqttActor::MqttMsg> UMqttActor::msgs;
std::vector<UMqttActor::FailData> UMqttActor::fails;

// Sets default values
UMqttActor::UMqttActor()
...
// Called every frame
void UMqttActor::Tick(float DeltaTime)
{
...
EMqttCode UMqttActor::DestroyClient()
{
...
EMqttCode UMqttActor::ConnectSSL(int32 In_iTimeoutSec, bool In_bClean, FString In_username, FString In_password, FString In_trustStore, FString In_keyStore, FString In_privateKey, FString In_privateKeyPassword, FString In_enabledCipherSuites, FString In_CApath, int32 In_enableServerCertAuth, int32 In_verify)
{
...
EMqttCode UMqttActor::Connect(int32 In_iTimeoutSec, bool In_bClean, FString In_username, FString In_password)
{
	conn_opts.context = this;
	conn_opts.onFailure = onfailure;
	conn_opts.connectTimeout = In_iTimeoutSec;
...
EMqttCode UMqttActor::Disconnect()
{
	EMqttCode err = EMqttCode::MQTT_SUCCESS;
...
EMqttCode UMqttActor::IsConnected()
{
...
EMqttCode UMqttActor::Start_Consuming()
{
...
EMqttCode UMqttActor::Stop_Consuming()
{
	EMqttCode err = IsConnected();
...
EMqttCode UMqttActor::Try_consume_message(FString& Out_sTopic, TArray<uint8>& Out_sMsg)
{
	EMqttCode err = IsConnected();
	bool bGotMsg(false);
...
EMqttCode UMqttActor::Subscribe(FString In_sTopic, int32 In_iQOS)
{
	EMqttCode err = IsConnected();
...
EMqttCode UMqttActor::UnSubscribe(FString In_sTopic)
{
	EMqttCode err = IsConnected();
...
EMqttCode UMqttActor::Publish(FString In_sTopic, FString In_sMsg, int32 In_iQOS)
{
	EMqttCode err = IsConnected();
...
EMqttCode UMqttActor::PublishBytes(FString In_sTopic, const TArray<uint8>& In_sMsg, int32 In_iQOS /*= 0*/)
{
	EMqttCode err = IsConnected();
...
EMqttCode UMqttActor::PublishObject(FString In_sTopic, UObject* In_sMsg, int32 In_iQOS /*= 0*/)
{
	EMqttCode err = IsConnected();
...
UObject* UMqttActor::ConvertBytesToUObject(TArray<uint8> Data, UClass* TargetClass)
{
...
AActor* UMqttActor::ConvertBytesToActor(TArray<uint8> Data, AActor* Target)
{
	if (Target == nullptr)
	{
		return nullptr;
...
TArray<uint8> UMqttActor::ConvertUObjectToBytes(UObject* Target)
{
	TArray<uint8> data;
	FMemoryWriter MemoryWriter(data, true);
	MemoryWriter.ArIsSaveGame = true;
...
FString UMqttActor::ConvertUObjectToJsonString(UObject* Target)
{
...
UObject* UMqttActor::ConvertJsonStringToUObject(FString Target, UClass* TargetClass)
{
...
AActor* UMqttActor::ConvertJsonStringToActor(FString Target, AActor* TargetActor)
{
	TSharedRef<TJsonReader<>> Reader = TJsonReaderFactory<>::Create(Target);
	TSharedPtr<FJsonObject> JsonObject;
	FJsonSerializer::Deserialize(Reader, JsonObject);
...
FString UMqttActor::ConvertToString(TArray<uint8> Data)
{
...
EMqttCode UMqttActor::PublishRaw(FString In_sTopic, char* In_pBuf, size_t In_BufSize, int32 In_iQOS)
{
	EMqttCode err = IsConnected();
